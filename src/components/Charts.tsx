...
export default function Charts({
  ...
  return (
    <>
        ...
        <div className="card p-4 mb-4">
          <div className="flex items-center gap-1 mb-2">
            {subDrilldownCategory ? (
              <>
                <button onClick={onBack} className="text-xs text-accent hover:underline">
                  📊 支出构成
                </button>
                <span className="text-xs text-slate-400">›</span>
                <button onClick={onSubBack} className="text-xs text-accent hover:underline">
              ...
              <h2 className="text-sm font-medium text-slate-200">支出构成</h2>
            )}
          </div>
          ...
          {subDrilldownCategory ? (
            subLoading ? (
              ...
              <div className="space-y-1.5 max-h-64 overflow-y-auto">
                {subTxs.map(tx => (
                  <div key={tx.id} className="flex items-center justify-between py-1.5 px-2 bg-[rgba(30,41,59,0.4)] rounded-lg text-sm">
                ...
                <div className="text-right text-xs text-slate-400 pt-1 border-t border-[rgba(71,85,105,0.2)]">
            ...
            <>
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2">
                {(drilldownCategory?.subCategories || pieData).map((d: any, i: number) => (
                  <span key={d.categoryName || d.name} className="text-xs text-slate-400">
                    <span
      ...
      {trendData.length > 1 && (
        <div className="card p-4 mb-4">
          <h2 className="text-sm font-semibold text-text-primary mb-3">近6月趋势</h2>
