...
export default function Budget() {
  ...
  if (loading) return <div className="px-4 py-6 text-center text-gray-400">加载中...</div>;

  return (
    <div className="px-4 py-6">
      <h1 className="text-lg font-semibold text-gray-800 mb-4">预算设置</h1>

      <div className="bg-white rounded-xl p-4 shadow-sm mb-4">
        <h2 className="text-sm font-medium text-gray-800 mb-3">月度总预算</h2>
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm">
        <h2 className="text-sm font-medium text-gray-800 mb-3">分类预算</h2>
        <div className="space-y-3">
          {expenseCategories.map(cat => (
            <div key={cat.id} className="flex items-center gap-2">
              <span className="text-lg w-8 text-center">{cat.icon}</span>
              <span className="text-sm text-gray-700 w-16">{cat.name}</span>
              <input
                type="text" inputMode="decimal"
